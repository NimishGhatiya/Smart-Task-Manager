...
export default function Dashboard() {
  ...
  const [filters, setFilters] = useState<Filters>({ priority: 'All', status: 'All', showBlocked: false });
  ...
  const myTasks = useMemo(() => tasks.filter(t => t.assigneeId === me?.id), [tasks, me]);
  ...
  const filtered = useMemo(() => {
    ...
    if (filters.showBlocked) list = list.filter(t => t.deps.length>0).filter(t => t.status !== 'Done');
    return list;
  }, [tasks, filters]);
